...
* Óbitos efetivos versus Projeção (ARIMA)
  ...
  ##   mutate(
  ##     xsdeaths = n - forecast,
  ##     color = ifelse(xsdeaths >= 0, "Above Forecast", "Bellow Forecast"),
  ##     p_score = round(n / forecast, 2)
  ##   ) %>% 
  ##   ggplot() +
  ##   geom_point( aes(x = by, y = n, color = color), size = 3, alpha = 1) +
  ##   geom_point( aes(x = by, y = forecast, color = color, shape = 22), size = 3, alpha = 1, pch = 22) + 
  ##   scale_color_manual(
  ##     values = c(
  ##       "Above Forecast" = "darkgreen", 
  ##       "Bellow Forecast" = "red"
